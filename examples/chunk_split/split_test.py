...
class LayoutSpliter:
    ...
    def split_text_with_title(self, data, 
                              begin_pattern = None) -> List:
        """
...
if __name__ == "__main__":
    ...
    layout_spliter = LayoutSpliter(
        begin_pattern=
        r'^([(（]材料|\d+[)、.]|（材料|[一二三四五六七八九十]+、|（[一二三四五六七八九十]+）|\d+、|\d+. |阅读下列材料|统计表:)'
        )
    
    text_spliter = MarkdownHeaderTextSplitter()

    layout_result = layout_spliter.split_text_with_title(
        layout_data, 
        begin_pattern = r'^\s*(?:.?)?(例|【例|（例|【解析|典型真题)\s*')
    # begin_pattern = r'^\s*(?:.?)?(例|【例|（例|【解析|典型真题)\s*'
    md_result = text_spliter.split_text(md_texts)

    with open('/home/yangcehao/RAG-Factory/examples/chunk_split/test1.json', 'w', encoding='utf-8') as f:
        json.dump(layout_result, f, ensure_ascii=False, indent=4)

    with open('/home/yangcehao/RAG-Factory/examples/chunk_split/test2.json', 'w', encoding='utf-8') as f:
