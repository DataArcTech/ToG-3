...
class MultiGPUConfigManager:
    ...
    def rewrite_path(self) -> str:
        """获取重写后的query路径"""
        return self.base_config['rewrite']['path']
...
async def write_result(rag, tester, example_data, file_path, gpu_id, test_round):
    """写入结果到指定文件"""
    ...
    print(f"🚀 GPU {gpu_id} 第 {test_round} 轮申论图检索测试开始，结果保存到: {file_path}")
    
    if os.path.exists(file_path):
        os.remove(file_path)
    
    with open(file_path, 'w', encoding='utf-8') as f:
        f.write(f"GPU {gpu_id} 第 {test_round} 轮申论图检索测试结果" + "\n")
        f.write("="*80 + "\n")
    ...
    for idx, example in enumerate(example_data):
        ...
        result = [doc for doc, score in result if score > rag_score_threshold]
        
        # 图检索
        graph_start_time = time.time()
        graph_result = await tester.run_tests([rewritten_query])
        graph_retrieval = graph_result[query_id]['evidence_items']
        # # 为图检索中的id赋值自增id
        # for idx, item in enumerate(graph_retrieval):
        #     item['id'] = idx
        ...
        graph_retrieval_rerank = await asyncio.to_thread(rag.rerank_context, question_content, rewritten_query, graph_retrieval)
        
        # graph_retrieval = extract_json_from_markdown(graph_retrieval)
        # graph_retrieval = await asyncio.to_thread(rag.rerank, query, graph_retrieval, k=10)
        ...
        # 生成答案
        graph_answer = await asyncio.to_thread(rag.graph_answer, question_content, materials, graph_retrieval_rerank[:10])
        ...
        example_result.append(
            Example_result(
                ...
                seed_nodes=graph_result[query_id]['seed_nodes'],
                entities=graph_result[query_id]['entities'],
                events=graph_result[query_id]['events'],
                graph_retrieval= [doc.to_dict() for doc in graph_retrieval_rerank[:10]],
                # rag_retrieval=[],
    ...
    with open(file_path, 'w', encoding='utf-8') as f:
        ...
        #     # print("result: " + str(example.to_dict()) + "\n")
        #     f.write(json.dumps(example.to_dict(), ensure_ascii=False, indent=2) + "\n")
    
    print(f"✅ GPU {gpu_id} 第 {test_round} 轮申论图检索测试完成，结果已保存")
...
async def run_single_gpu_test(gpu_id: int, test_round: int, config_manager: MultiGPUConfigManager, 
    ...
    try:
        ...
        await write_result(rag, tester, example_data, file_path, gpu_id, test_round)
        
        return True
        
    except Exception as e:
        print(f"❌ GPU {gpu_id} 第 {test_round} 轮申论图检索测试失败: {e}")
        import traceback
        traceback.print_exc()
        return False
...
async def run_multi_gpu_tests(gpu_ids: list, test_rounds: int, config_path: str, 
    ...
    example_data_path = config_manager.rewrite_path()
    ...
    for gpu_id in gpu_ids:
        ...
        for round_num in range(1, test_rounds + 1):
            task = run_single_gpu_test(
                ...
                gpu_rag_systems[gpu_id], gpu_testers[gpu_id]
            )
            tasks.append(task)
...
async def main():
    """主函数"""
    # 配置参数
    gpu_ids = [0, 1,2,4,5,6,7]  # 要使用的GPU设备ID
    test_rounds = 1  # 每个GPU的测试轮次
    query_id_count = 17 # 要测试前多少道题目
    config_path = "/finance_ML/liuyingqi/RAG-Factory/examples/TCL_rag/config_commentary.yaml"
    example_data_path = "/data/FinAi_Mapping_Knowledge/liuyingqi/example/申论真题2020-2024.json"
    base_result_path = "/data/FinAi_Mapping_Knowledge/liuyingqi/commentary_test/graph_answer/answer_by_graph_commentary_multi_gpu.json"
