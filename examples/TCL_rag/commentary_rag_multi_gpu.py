...
async def write_result(rag, example_data, file_path, gpu_id, test_round):
    ...
    print(f"🚀 GPU {gpu_id} 第 {test_round} 轮申论RAG测试开始，结果保存到: {file_path}")
    
    if os.path.exists(file_path):
        os.remove(file_path)
    
    with open(file_path, 'w', encoding='utf-8') as f:
        f.write(f"GPU {gpu_id} 第 {test_round} 轮申论RAG测试结果" + "\n")
        f.write("="*80 + "\n")
    ...
    for idx, example in enumerate(example_data):
        ...
        result = await asyncio.to_thread(rag.invoke, query, k=20)
        result = await asyncio.to_thread(rag.rerank, query, result, k=10)
        docs = [doc for doc, score in result if score > rag_score_threshold]
        rag_answer = await asyncio.to_thread(rag.rag_answer, query, materials, docs)
        ...
        example_result.append(
            Example_result(
                ...
                rag_retrieval=[{'doc': doc.to_dict(), 'score': score} for doc, score in result if score > rag_score_threshold],
    ...
    print(f"✅ GPU {gpu_id} 第 {test_round} 轮申论RAG测试完成，结果已保存")
...
async def run_single_gpu_test(gpu_id: int, test_round: int, config_manager: MultiGPUConfigManager, 
    ...
    try:
        ...
        print(f"❌ GPU {gpu_id} 第 {test_round} 轮申论RAG测试失败: {e}")
        import traceback
        traceback.print_exc()
        return False
...
async def main():
    ...
    base_result_path = "/data/FinAi_Mapping_Knowledge/liuyingqi/commentary_test/rag_answer/answer_by_rag_commentary_multi_gpu.json"
