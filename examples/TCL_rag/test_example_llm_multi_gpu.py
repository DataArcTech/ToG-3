...
import re
from prompt import ANALYZE_LLM_PROMPT
...
class Example_result:
    ...
    def to_dict(self):
        return {
            ...
            "llm_is_correct": self.llm_is_correct
        }
...
async def write_result(rag, example_data, file_path, gpu_id, test_round, query_id_list, correct_count, total_count, error_file_name):
    ...
    print(f"🚀 GPU {gpu_id} 第 {test_round} 轮行测LLM测试开始，结果保存到: {file_path}")
    
    if os.path.exists(file_path):
        os.remove(file_path)
    
    with open(file_path, 'w', encoding='utf-8') as f:
        f.write(f"GPU {gpu_id} 第 {test_round} 轮行测LLM测试结果" + "\n")
        f.write("="*80 + "\n")
    ...
    for example in example_data:
        ...
        llm_answer = await asyncio.to_thread(rag.llm_answer, query)
        llm_end_time = time.time()
        llm_spend_time = llm_end_time - llm_start_time
        
        llm_answer = extract_json_from_markdown(llm_answer)
        llm_answer_dict = json.loads(llm_answer) if isinstance(llm_answer, str) else llm_answer
        
        # 统计正确性
        if not llm_answer_dict['answer'] == answer:
        ...
        example_result.append(
            Example_result(
                ...
                llm_is_correct=llm_answer_dict['answer'] == answer
            )
        )
    ...
    print(f"✅ GPU {gpu_id} 第 {test_round} 轮行测LLM测试完成，结果已保存")
...
async def run_single_gpu_test(gpu_id: int, test_round: int, config_manager: MultiGPUConfigManager, 
    ...
    try:
        ...
        print(f"❌ GPU {gpu_id} 第 {test_round} 轮行测LLM测试失败: {e}")
        import traceback
        traceback.print_exc()
        return False
...
async def run_multi_gpu_tests(gpu_ids: list, test_rounds: int, config_path: str, 
    ...
    with open(result_path, 'a', encoding='utf-8') as f:
        f.write("\n" + "="*80 + "\n")
        f.write(f"prompt: {ANALYZE_LLM_PROMPT}\n")
        f.write("\n" + "="*40 + "\n")
...
async def main():
    ...
    base_result_path = "/data/FinAi_Mapping_Knowledge/liuyingqi/llm_answer_test/answer_by_llm_multi_gpu.json"
    result_path = "/data/FinAi_Mapping_Knowledge/liuyingqi/llm_answer_test/4.1_result.txt"
