    ...
    for line in lines:
        for span in line['spans']:
            if "equation" in span['type']:
                return True
            if "=" in span.get("content",'') or "公式" in span.get("content",''):
                return True
...
def use_dophin_analysis_equation(pdf_data: Union[str, bytes], page_id: int, index: int, bbox: list, model:DOLPHIN) -> str:
    ...
    # pil_image.save(f'/home/yangcehao/edu_project/documents_analysis/test1_result/2026国考公务员行测-资料部分1/auto/images/{page_id}_{index}.png')
